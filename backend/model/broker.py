...
class BrokerMonitoring(Base):
    __tablename__ = 'broker_monitoring'
    id = Column(Integer, primary_key=True, autoincrement=True)
    network_usage = Column(BigInteger, nullable=False)
    cpu_usage = Column(Integer, nullable=False)
    last_updated = Column(DateTime, nullable=False)
    broker_id = Column(String(100), ForeignKey(Broker.id))
